...
class App extends Component {
  ...
  componentDidMount = () => {
    ...
    //     that1.setState({ localRearStream: mediaStream });
    //   }).catch((err) => {
    ...
    this.signaling.on('peers', (peers, self) => {
      ...
      let ourRoom = peers.filter((item) => {
        ...
        // return (item.id !== self && item.iframe_url === this.getParentURL())
        return (item.id !== self)
      })
      const that = this;
  ...
  handleStart = () => {
    ...
    if (ourRoom.length > 1) {
      ...
      this.signaling.getLocalStream('video', this.state.camera_front).then((stream) => {
  ...
  render() {
    ...
    // this.signaling.getLocalStream('video', true)
    // .then( (stream) => {
    // })
    // .catch((err) => {
    //   // alert("I can't find requested devices. Please check your camera and mic")
    // });
        ...
        <div className={classes.root}>
          ...
          >
            ...
            <div style={styles.noJoin}>
              {/* <p style={{position: "absolute", zIndex: "-1", top: "45%", left: "50%", transform: "translate(-50%,-50%)",}}>Please wait until Client join</p> */}
              {
              ...
              {
                this.state.localRearStream != null ? <LocalRearVideoView stream={this.state.localRearStream} ToggleAudio={this.onAudioClickHandler} muted={this.state.video_muted} id={'localrearview'} /> : null
              }
