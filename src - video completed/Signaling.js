...
export default class Signaling extends events.EventEmitter {
    ...
    onSwitchCamera = (peer_id, media, shouldFaceUser) => {
        console.log('switch camera', peer_id, shouldFaceUser);
        this.getLocalStream('video', shouldFaceUser).then((stream) => {
            // this.createPeerConnection(peer_id, 'video', false, stream);
            this.emit('switch_camera', stream);
        })
        // media = 'video';
        // this.session_id = this.self_id + '-' + peer_id;
        ...
        //     this.emit('localstream', stream);
        //     this.emit('new_call', this.self_id, this.session_id);
        //     this.createPeerConnection(peer_id, media, false, stream);
        // });
    }
    ...
    createPeerConnection = (id, media, isOffer, localstream) => {
        ...
        if (isOffer) {
            // pc.onnegotiationneeded = () => {
            //     console.log('onnegotiationneeded when isOffer is true');
            // }
            this.createOffer(pc, id, media);
        }
        return pc;
    ...
    onOffer = (message) => {
        ...
        this.getLocalStream(media).then((stream) => {
            ...
            // pc.onaddstream = (event) => {
            //     console.log('onaddstream on offer', event);
            //     this.emit('addstream', event.stream);
            // };
            // pc.addStream(stream);
